...
namespace YoutubePlayer
{
    public static class VideoPlayerExtensions
    {
        ...
        {
            YoutubeVideoMetaData metaData = await YoutubeDl.GetVideoMetaDataAsync<YoutubeVideoMetaData>(youtubeUrl, YoutubeDlOptions.Default, k_PlayFields, YoutubeDlCli.YtDlp, cancellationToken);
            string rawUrl = metaData.Url;

            videoPlayer.source = VideoSource.Url;

            //Resetting the same url restarts the video...
            if (videoPlayer.url != rawUrl) { videoPlayer.url = rawUrl; }
